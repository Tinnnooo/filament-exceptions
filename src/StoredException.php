...
class StoredException
{
    ...
    protected function decodeTrace(): array
    {
        ...
        if (is_string($trace)) {
            ...
            if ($decoded === null) {
                $decoded = $this->parseStackTraceString($trace);
    ...
    protected function parseStackTraceString(string $trace): array
    {
        $frames = [];

        // Split lines, ignore empties
        $lines = preg_split("/\R/", trim($trace));
        foreach ($lines as $line) {
            $line = trim($line);

            // Match: #0 /path/file.php(794): Some\Class->method()
            if (! preg_match('/^#(?P<index>\d+)\s+(?P<file>.+?)\((?P<line>\d+)\):\s+(?P<call>.+)$/', $line, $m)) {
                // Match: #21 {main}
                if (preg_match('/^#(?P<index>\d+)\s+\{main\}$/', $line, $m2)) {
                    $frames[] = [
                        'index' => (int) $m2['index'],
                        'file' => null,
                        'line' => null,
